...
<template>
  <UCard class="max-w-sm w-full">
    <div class="text-center mb-6">
      <UIcon name="i-heroicons-identification" class="mb-2 dark:text-white w-8 h-8" />
      <p class="text-2xl dark:text-white font-bold">Lets signup!</p>
      <p class="dark:text-gray-400 mt-1"> Follow the steps below. </p>
    </div>
    <UForm v-if="!isValid" :schema="schema" :state="state" class="gap-y-6 space-y-4" @submit="verifyCode">
      <UFormGroup label="Invite Code" name="code">
        <UInput v-model="state.code" placeholder="Enter your invite code" />
      </UFormGroup>
    ...
    <UForm v-else :schema="schema" :state="state" class="gap-y-6 space-y-4" @submit="onSubmit">
      <UFormGroup label="Invite code" name="code">
        <UInput v-model="state.code" :placeholder="state.code" />
      </UFormGroup>
      ...
      <UFormGroup label="Re-enter password" name="repassword">
        <UInput v-model="state.repassword" placeholder="Password" />
      </UFormGroup>
      <UButton label="Submit" type="submit" block />
      <span class="flex text-sm dark:text-gray-400 mt-2 text-center">We will never share your email with anyone
        else.</span>
