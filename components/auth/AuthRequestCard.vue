...
<template>
  <UCard class="max-w-sm w-full">
    ...
    <UForm :schema="schema" :state="state" class="gap-y-6 space-y-4" @submit="onSubmit">
      ...
      <UFormGroup label="Do you know an existing user?" name="toggle">
        <UToggle v-model="state.toggle" />
      </UFormGroup>
      <UFormGroup v-if="state.toggle" label="Name of the user" name="username" required>
        <UInput v-model="state.username" placeholder="User" />
      </UFormGroup>
      <UFormGroup label="Text Area" name="textarea" required>
